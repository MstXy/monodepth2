...
class ColorJitter(tf.ColorJitter):
    def __call__(self, imgs):
        ...
        _, brightness_factor, contrast_factor, saturation_factor, hue_factor = self.get_params(self.brightness, self.contrast, self.saturation, self.hue)
        transforms = []

        if brightness_factor is not None:
            transforms.append(tf.Lambda(lambda img: F.adjust_brightness(img, brightness_factor)))

        if contrast_factor is not None:
            transforms.append(tf.Lambda(lambda img: F.adjust_contrast(img, contrast_factor)))

        if saturation_factor is not None:
            transforms.append(tf.Lambda(lambda img: F.adjust_saturation(img, saturation_factor)))

        if hue_factor is not None:
            transforms.append(tf.Lambda(lambda img: F.adjust_hue(img, hue_factor)))
