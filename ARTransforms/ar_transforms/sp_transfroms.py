...
from monodepth2.ARTransforms.ar_transforms.interpolation import Interp2
from monodepth2.ARTransforms.ar_transforms.interpolation import Meshgrid
import numpy as np
...
class RandomAffineFlow(nn.Module):
    def __init__(self, cfg, addnoise=True):
        ...
        self._random_mirror = RandomMirror(cfg.st_vflip) if cfg.st_hflip else RandomMirror(p=1)
    ...
    def forward(self, data):
        ...
        theta_list = [self.apply_random_transforms_to_params(
            theta0,
            max_translate=self.cfg.st_trans[0],
            min_zoom=self.cfg.st_zoom[0], max_zoom=self.cfg.st_zoom[1],
            min_squeeze=self.cfg.st_squeeze[0], max_squeeze=self.cfg.st_squeeze[1],
            min_rotate=self.cfg.st_rotate[0], max_rotate=self.cfg.st_rotate[1],
        ...
        for i in range(len(imgs) - 1):
            theta_list.append(
                self.apply_random_transforms_to_params(
                    theta_list[-1],
                    max_translate=self.cfg.st_trans[1],
                    min_zoom=self.cfg.st_zoom[2], max_zoom=self.cfg.st_zoom[3],
                    min_squeeze=self.cfg.st_squeeze[2], max_squeeze=self.cfg.st_squeeze[3],
                    min_rotate=self.cfg.st_rotate[2], max_rotate=self.cfg.st_rotate[2],
                    validate_size=[height, width])
            )
