...
class unFlowLoss(nn.modules.Module):
    ...
    def loss_photomatric(self, im1_scaled, im1_recons, occu_mask1, scale=0):
        ...
        tmp1 = torch.sum(loss[0][0], dim=0, keepdim=True) / (torch.max(torch.sum(loss[0][0], dim=0)) + 1e-5)
        tmp2 = torch.sum(loss[1][0], dim=0, keepdim=True) / (torch.max(torch.sum(loss[1][0], dim=0)) + 1e-5)
        tmp3 = torch.sum(loss[2][0], dim=0, keepdim=True) / (torch.max(torch.sum(loss[2][0], dim=0)) + 1e-5)
        ...
        # nan check
        # print("torch.max(torch.abs(im1_recons), ", torch.max(torch.abs(im1_recons)))
        # print("loss[0].mean()",loss[0].mean())
        # print("loss[1].mean()",loss[1].mean())
        # print("occu_mask1.mean()",occu_mask1.mean())
        # print("loss[2][0]", loss[2][0].mean())
