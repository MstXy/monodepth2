...
import datasets.flow_eval_datasets as flow_eval_datasets
import monodepth2.utils as mono_utils
    ...
    def val(self):
        ...
        with torch.no_grad():
            ...
            for i, metric in enumerate(self.depth_metric_names):
                print("depth_metric:", metric, 'is', losses[metric])

            self.log("val", inputs, outputs, losses)
            del inputs, outputs, losses
            
            self.val_data_root = '/mnt/km-nfs/ns100002-share/data_scene_flow'
            out_list, epe_list = [], []
            ...
            for occ_noc in ['flow_occ', 'flow_noc']:
                ...
                val_dataset = flow_eval_datasets.KITTI_2015_scene_flow(split='training', root=self.val_data_root, occ_noc=occ_noc)
                save_path_dir = os.path.join(self.log_path, occ_noc+'evaluate_flow_kitti')
                
                os.makedirs(save_path_dir, exist_ok=True)
                for val_id in range(len(val_dataset)):
                    init_input_dict = val_dataset[val_id]
                    # {('color', -1, 0): img1, ('color', 0, 0): img2, ('flow', -1, 0): flow, ('valid', -1, 0): valid}
                    image1_ori, image2_ori, flow_gt, valid_gt = init_input_dict[('color', -1, 0)], init_input_dict[('color', 0, 0)], \
                                                                init_input_dict[('flow', -1, 0)], init_input_dict[('valid', -1, 0)]   
                    _, H, W = image1_ori.shape
                    image1_ori = image1_ori[None].to(self.device)
                    image2_ori = image2_ori[None].to(self.device)
                    flow_gt = flow_gt.to(self.device)
                    valid_gt = valid_gt.to(self.device)
                    # if opt.norm_trans:
                    ...
                    imgs = [input_dict[("color_aug", i, 0)] for i in self.opt.frame_ids] # all images: ([i-1, i, i+1])
                    features = [self.models["encoder"](img) for img in imgs]
                    outputs = self.models["depth"](features[0]) # depth only takes in current frame
                    outdict = self.models["flow"](imgs, features)
                    pyramid_flows = []
                    for idx_pair in self.idx_pair_list:
                        for scale in self.opt.scales:
                            pyramid_flows.append(
                                torch.cat(
                                (outdict[('flow', idx_pair[0], idx_pair[1], scale)], outdict[('flow', idx_pair[1], idx_pair[0], scale)]), 
                                dim=1)
                                )

                    flow_1_2 = outdict[('flow', -1, 0, 0)][0]
                    ...
                    print("epe_tmp=", epe_tmp, "out_tmp=", 100 * np.mean(out_tmp))
    ...
    def compute_losses(self, inputs, outputs):
        ...
        if self.use_flow:
            ...
            for idx_pair in self.idx_pair_list:
                ...
                flow_loss_tmp, l_ph_pyramid, l_sm_pyramid, flow_mean, pyramid_occ_mask1, pyramid_occ_mask2, \
                    pyramid_im1_recons, pyramid_im2_recons = self.arflow_loss(pyramid_flows, target)
                flow_loss += flow_loss_tmp
...
if __name__ == "__main__":
    ...
    opts = options.parse()
    opts.load_weights_folder='/mnt/km-nfs/ns100002-share/zcy-exp/tmp/mdp/1020_2/mdp/models/weights_67'
    opts.log_dir='/mnt/km-nfs/ns100002-share/zcy-exp/Evaluation'
    eval = Eval(opts, parser=options.parser)
    eval.val()