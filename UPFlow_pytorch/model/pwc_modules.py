...
# import sys
# sys.path.append('/home/liu/data16t/Projects/depth_estim/UPFlow_pytorch')


# from UPFlow_pytorch.utils.tools import tools
...
def get_grid(x):
    ...
    if x.is_cuda:
        grids_cuda = grid.float().requires_grad_(False).to(device)
    else:
        grids_cuda = grid.float().requires_grad_(False)  # .to(device)
    return grids_cuda
...
class WarpingLayer(nn.Module):
    ...
    def forward(self, x, flow, height_im, width_im, div_flow):
        ...
        if x.is_cuda:
            mask = torch.ones(x.size(), requires_grad=False).to(device)
        else:
            mask = torch.ones(x.size(), requires_grad=False)  # .to(device)
