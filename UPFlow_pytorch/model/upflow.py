...
import monodepth2.options as tmp_options
...
class UPFlow_net(tools.abstract_model):
    ...
    def __init__(self, conf: config):
        ...
        self.conv_1x1_v2 = nn.ModuleList([conv(512, 32, kernel_size=1, stride=1, dilation=1),
                                       conv(256, 32, kernel_size=1, stride=1, dilation=1),
                                       conv(128, 32, kernel_size=1, stride=1, dilation=1),
                                       conv(64, 32, kernel_size=1, stride=1, dilation=1),
                                       conv(64, 32, kernel_size=1, stride=1, dilation=1)])
        ...
        from monodepth2.networks.correlation.correlation import FunctionCorrelation
        self.corr_block = FunctionCorrelation

    def forward(self, input_dict: dict):
        '''
        :param input_dict:     x1_features, x2_features, im1, im2, im1_raw, im2_raw, start, if_loss, if_shared_features
        :return: output_dict:  flows, flow_f_out, flow_b_out, photo_loss
        '''
        if_shared_features = input_dict['if_shared_features']
        ...
        output_dict = {}
        if if_shared_features:
            x1_features = input_dict['x1_features']
            x2_features = input_dict['x2_features']
            flow_f_pwc_out, flow_b_pwc_out, flows = self.upflow_decoder_two_frames(x1_features, x2_features, im1, im2)  # forward estimation
        else:
            flow_f_pwc_out, flow_b_pwc_out, flows = self.forward_2_frame_v3(im1, im2)  # forward estimation
    ...
    def upflow_decoder_two_frames(self, x1_features, x2_features, x1_raw, x2_raw):
        _, _, height_im, width_im = x1_raw.size()
        # on the bottom level are original images

        x1_pyramid = x1_features[::-1] + [x1_raw]
        x2_pyramid = x2_features[::-1] + [x2_raw]
        ...
        for l, (x1, x2) in enumerate(zip(x1_pyramid, x2_pyramid)):
            x1_1by1 = self.conv_1x1_v2[l](x1)
            x2_1by1 = self.conv_1x1_v2[l](x2)
    ...
    def forward_2_frame_v3(self, x1_raw, x2_raw):
        _, _, height_im, width_im = x1_raw.size()
        # on the bottom level are original images
    ...
    def decode_level_res(self, level, flow_1, flow_2, feature_1, feature_1_1x1, feature_2, feature_2_1x1, img_ori_1, img_ori_2):
        ...
        #     out_corr_2 = self.correlation_pytorch(feature_2, feature_1_warp)
            out_corr_1 = self.corr_block(feature_1, feature_2_warp)
            out_corr_2 = self.corr_block(feature_2, feature_1_warp)

        else:
            corr_func1 = Correlation(pad_size=self.search_range, kernel_size=1, max_displacement=self.search_range, stride1=1, stride2=1, corr_multiply=1)
            corr_func2 = Correlation(pad_size=self.search_range, kernel_size=1, max_displacement=self.search_range, stride1=1, stride2=1, corr_multiply=1)
            out_corr_1 = corr_func1(feature_1, feature_2_warp)
            out_corr_2 = corr_func2(feature_2, feature_1_warp)
    ...
    def demo(cls):
        ...
        im = np.random.random((12, 3, 196, 640))
        start = np.zeros((1, 2, 1, 1))
        start = torch.from_numpy(start).float().to(device)
        im_torch = torch.from_numpy(im).float().to(device)
