...
import monodepth2.networks.raft.raft as raft
...
class MonoFlowNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        all_color_aug = torch.cat(
            [inputs[("color_aug", i, 0)] for i in self.opt.frame_ids])  # [0, -1, 1] or [0, -1]
        all_features = self.Encoder(all_color_aug)
        all_features = [torch.split(f, curr_bs) for f in all_features]  # separate by frame
        
        features = {}
        for i, k in enumerate(self.opt.frame_ids): # (i,k) = (0, 0), (1, -1), (2, 1)
            features[k] = [f[i] for f in all_features]  
            # f[0] is the feature of frame 0, f[1] is the feature of frame -1, f[2] is the feature of frame 1
    ...
    def predict_flow(self, inputs, features):
        ...
        if self.opt.optical_flow in ['flownet', ]:
            ...
            imgs = [inputs[("color_aug", i, 0)] for i in self.opt.frame_ids] # 0, -1, 1
            in_feat = []
            ...
            for (img1_idx, img2_idx) in idx_pair_list:
                ...
                feature_pair = [features[img1_idx][1], features[img2_idx][1]]
                
                out_1 = self.FlowDecoder(img_pair, feature_pair, self.opt.raft_iter)
                out_2 = self.FlowDecoder(img_pair[::-1], feature_pair[::-1], self.opt.raft_iter)
    ...
    def build_flow_decoder(self):
        if self.opt.optical_flow in ["flownet", ]:
            ...
            return None, raft.RAFT(self.opt)        
        else: 
            raise NotImplementedError
