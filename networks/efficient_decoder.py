...
from layers import *
import math
from networks.PolarizedSelfAttention import *
...
class Fppsa(nn.Module):
    def __init__(self, num_ch, top=False):
        super(Fppsa, self).__init__()
        self.top = top
        self.swish = MemoryEfficientSwish()
        ...
        self.up_conv = SeparableConvBlock(num_ch)
        self.conv = SeparableConvBlock(num_ch)
        self.cat_conv = SequentialPolarizedSelfAttention(num_ch)
        self.iconv = SeparableConvBlock(num_ch)
...
class EfficientDecoder(nn.Module):
    def __init__(self, num_ch_enc, epsilon=1e-4):
        ...
        self.disp0 = nn.Sequential(Conv3x3(bottleneck//2, 1), nn.Sigmoid())

    # @profile
    def forward(self, input_features, f_i=0):
        self.outputs = {}
        # l1, l2, l3, l4, l5, l6 = input_features
        ...
        x0 = self.swish(self.conv1(self.attn_0(x0)))
