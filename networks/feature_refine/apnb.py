...
class APNB(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, key_channels, value_channels, dropout, sizes=([1]), norm_type=None,psp_size=(1,3,6,8)):
        ...
        self.conv_bn_dropout = nn.Sequential(
            ...
            BNReLU(out_channels, norm_type=norm_type),
            ...
            if not isinstance(frame_id, str): # not right view
                for idx in range(5):
                    if idx in [4]: # only the last layer?
                        ...
                        priors = [stage(tmp) for stage in self.stages]
                        context = priors[0]
                        for i in range(1, len(priors)):
                            context += priors[i]
                        output = self.conv_bn_dropout(torch.cat([context, tmp], 1))

                        x[frame_id][idx] = output
                        return x
