...
class PWCDecoder_from_img(nn.Module):
    ...
    def __init__(self, md=4, training=True):
        ...
        self.up_feat1 = deconv(od+dd[4], 2, kernel_size=4, stride=2, padding=1)
    ...
    def forward(self, im1, im2):
        ...
        flow6 = flow6 + self.context_net(torch.cat((x_intm, flow6), dim=1))
        up_flow6 = F.interpolate(flow6 * 2, scale_factor=2, mode='bilinear', align_corners=True)
        # up_feat6 = self.upfeat6(x)

        
        warp5 = self.warp_out(c25, up_flow6*0.625)
        ...
        warp4 = self.warp_out(c24, up_flow5*1.25)
        ...
        warp3 = self.warp_out(c23, up_flow4*2.5)
        ...
        warp2 = self.warp_out(c22, up_flow3*5.0) 
